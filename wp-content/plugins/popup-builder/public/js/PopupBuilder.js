...
{this.id=null;this.eventName='';this.popupData=null;this.popupConfig={};this.popupObj=null;this.onceListener();this.initialsListeners();}
...
var lastPopupId=parseInt(currentPopup['popupId']);SGPBPopup.closePopupById(lastPopupId);}};});sgAddEvent(window,'sgpbDidClose',function(e){if(window.sgPopupBuilder.length!=0){var popups=[].concat(window.sgPopupBuilder).reverse();for(var i in popups){var nextIndex=++i;var nextObj=popups[nextIndex];if(typeof nextObj=='undefined'){jQuery('html').css({overflow:'auto'});break;}
if(nextObj.isOpen==false){continue;}
var options=SGPBPopup.getPopupOptionsById(nextObj.popupId);if(typeof options['sgpb-disable-page-scrolling']=='undefined'){jQuery('html').css({overflow:'auto'});}
else{jQuery('html').css({overflow:'hidden'});}
break;}}
else{jQuery('html').css({overflow:'auto'});}});};SGPBPopup.prototype.getPopupIdForNextEsc=function()
...
{var popupId=this.getPopupId();var popupData=this.getPopupData();var popupZIndex=this.getCompatibleZiIndex(popupData['sgpb-popup-z-index']);this.setUpPopupConfig();var that=this;var popupConfig=this.getPopupConfig();this.setPopupDimensions();if(popupData['sgpb-disable-popup-closing']=='on'){popupData['sgpb-enable-close-button']='';popupData['sgpb-esc-key']='';popupData['sgpb-overlay-click']='';}
popupData['eventName']=this.eventName;if(SGPBPopup.varToBool(popupData['sgpb-enable-close-button'])){popupConfig.magicCall('setCloseButtonDelay',parseInt(popupData['sgpb-close-button-delay']));}
popupConfig.magicCall('setShowButton',SGPBPopup.varToBool(popupData['sgpb-enable-close-button']));var openAnimationSpeed=parseInt(popupData['sgpb-open-animation-speed'])*1000;popupConfig.magicCall('setOpenAnimationEffect',popupData['sgpb-open-animation-effect']);popupConfig.magicCall('setOpenAnimationSpeed',openAnimationSpeed);popupConfig.magicCall('setOpenAnimationStatus',popupData['sgpb-open-animation']);popupConfig.magicCall('setContentPadding',popupData['sgpb-content-padding']);popupConfig.magicCall('setZIndex',popupZIndex);popupConfig.magicCall('setCloseButtonWidth',popupData['sgpb-button-image-width']);popupConfig.magicCall('setCloseButtonHeight',popupData['sgpb-button-image-height']);popupConfig.magicCall('setPopupId',popupId);popupConfig.magicCall('setPopupData',popupData);popupConfig.magicCall('setAllowed',!SGPBPopup.varToBool(popupData['sgpb-disable-popup-closing']));if(popupData['sgpb-type']==SGPB_POPUP_PARAMS.popupTypeAgeRestriction){popupConfig.magicCall('setAllowed',false);}
...
this.themeCreator();this.themeCustomizations();popupConfig.magicCall('setContents',document.getElementById('sg-popup-content-wrapper-'+popupId));this.setPopupConfig(popupConfig);this.popupTriggeringListeners();var allowToOpen=this.checkCurrentPopupType();if(allowToOpen){this.open();}};SGPBPopup.prototype.checkCurrentPopupType=function()
...
return allowToOpen;};SGPBPopup.prototype.popupLimitation=function(popupData)
...
else{var popupLimitationPageLevelCookie=jQuery.parseJSON(popupLimitationPageLevelCookie);if(popupLimitationPageLevelCookie){SGPBPopup.deleteCookie(popupLimitationCookiePageLevelName,popupLimitationPageLevelCookie.openingPage);}
...
{var noPositionSelected=false;var popupData=this.getPopupData();var popupId=this.getPopupId();var popupConfig=this.getPopupConfig();var popupTheme=popupData['sgpb-popup-themes'];var closeButtonWidth=popupData['sgpb-button-image-width'];var closeButtonHeight=popupData['sgpb-button-image-height'];var contentPadding=parseInt(popupData['sgpb-content-padding']);var contentClass=popupData['sgpb-content-custom-class'];var closeButtonImage=popupConfig.closeButtonImage;var themeNumber=1;if(popupData['sgpb-type']=='countdown'){popupConfig.magicCall('setMinWidth',300);}
popupConfig.magicCall('setContentPadding',contentPadding);popupConfig.magicCall('setOverlayAddClass',popupTheme+'-overlay sgpb-popup-overlay-'+popupId);popupConfig.magicCall('setContentAddClass','sgpb-content sgpb-content-'+popupId+' '+popupTheme+'-content '+contentClass);if(typeof popupData['sgpb-close-button-position']=='undefined'||popupData['sgpb-close-button-position']==''){var noPositionSelected=true;}
...
else if(popupTheme=='sgpb-theme-2'){themeNumber=2;popupConfig.magicCall('setButtonInside',false);popupConfig.magicCall('setContentBorderWidth',1);popupConfig.magicCall('setContentBackgroundColor','black');popupConfig.magicCall('setContentBorderColor','black');popupConfig.magicCall('setOverlayColor','white');if(noPositionSelected||closeButtonPosition=='topRight'){popupConfig.magicCall('setCloseButtonPositionRight','0');popupConfig.magicCall('setCloseButtonPositionTop','-'+closeButtonHeight+'px');}
...
else{popupConfig.magicCall('setButtonImage',popupData['sgpb-button-image']);}};SGPBPopup.prototype.themeCustomizations=function()
{var popupId=this.getPopupId();var popupData=this.getPopupData();var popupConfig=this.getPopupConfig();var contentOpacity=popupData['sgpb-content-opacity'];var contentBgColor=popupData['sgpb-background-color'];var contentBgImage=popupData['sgpb-background-image'];var showContentBackground=popupData['sgpb-show-background'];var contentBgImageMode=popupData['sgpb-background-image-mode'];var overlayColor=popupData['sgpb-overlay-color'];var popupTheme=popupData['sgpb-popup-themes'];if(typeof showContentBackground=='undefined'){contentBgColor='';contentBgImage='';contentBgImageMode='';}
...
popupContent.find('iframe').each(function(){var src=jQuery(this).data('src');if(typeof src=='undefined'){return true;}
if(popupEventName!='open'){jQuery(this).attr('data-attr-src',src);jQuery(this).attr('src','');return true;}
...
{var that=this;var popupData=this.getPopupData();var popupConfig=this.getPopupConfig();sgAddEvent(window,'sgpbDidOpen',function(e){var args=e.detail;that.formSubmissionDetection(args);var popupOptions=args.popupData;if(typeof SGPB_ANALYTICS_PARAMS=='undefined'){that.addToCounter(popupOptions['sgpb-post-id']);}
...
{var dontShowPopup=jQuery('#sg-popup-content-wrapper-'+popupId+' [class^=sg-popup-dont-show]');if(!dontShowPopup.length){return false;}
...
var params={};params.popupId=popupId;var data={action:'sgpb_send_to_open_counter',nonce:SGPB_JS_PARAMS.nonce,params:params};jQuery.post(SGPB_JS_PARAMS.ajaxUrl,data,function(res){});};SGPBPopup.prototype.closeButtonDisplay=function(popupId,display,delay)
{if(display=='show'){setTimeout(function(){jQuery('.sgpb-content-'+popupId).prev().show();},delay*1000);}
else if(display=='hide'){jQuery('.sgpb-content-'+popupId).prev().hide();}};SGPBPopup.prototype.open=function()
...
{var status=false;var popupContentMainDiv=document.getElementById('sgpb-popup-dialog-main-div');while(targetName.parentNode){targetName=targetName.parentNode;var tagName=targetName.tagName.toLowerCase();if(targetName===popupContentMainDiv){break;}
if(exceptionList.indexOf(tagName)!=-1){status=true;break;}}
return status;};SGPBPopup.prototype.contentCloseBehavior=function()
{var that=this;var popupData=this.getPopupData();var popupId=this.getPopupId();var redirectUrl=popupData['sgpb-click-redirect-to-url'];var contentClickBehavior=popupData['sgpb-content-click-behavior'];var redirectToNewTab=SGPBPopup.varToBool(popupData['sgpb-redirect-to-new-tab']);sgAddEvent(window,'sgpbDidOpen',function(e){});sgAddEvent(window,'sgpbWillOpen',function(e){if(popupId!=e.detail.popupId||e.detail.popupData['sgpb-content-click']=='undefined'){return false;}
...
{var popupData=this.getPopupData();var popupConfig=this.getPopupConfig();var popupId=this.getPopupId();var dimensionData=popupData['sgpb-popup-dimension-mode'];var maxWidth=popupData['sgpb-max-width'];var maxHeight=popupData['sgpb-max-height'];var minWidth=popupData['sgpb-min-width'];var minHeight=popupData['sgpb-min-height'];var contentPadding=popupData['sgpb-content-padding'];popupConfig.magicCall('setMaxWidth',maxWidth);popupConfig.magicCall('setMaxHeight',maxHeight);popupConfig.magicCall('setMinWidth',minWidth);popupConfig.magicCall('setMinHeight',minHeight);if(dimensionData=='responsiveMode'){var dimensionMeasure=popupData['sgpb-responsive-dimension-measure'];var popupConfig=this.getPopupConfig();if(dimensionMeasure!='auto'){popupConfig.magicCall('setWidth',dimensionMeasure+'%');}
else{popupConfig.magicCall('setWidth',jQuery('.sgpb-popup-builder-content-'+popupId).width()+(contentPadding*2)+'px');}
return popupConfig;}
var popupWidth=popupData['sgpb-width'];var popupHeight=popupData['sgpb-height'];popupConfig.magicCall('setWidth',popupWidth);popupConfig.magicCall('setHeight',popupHeight);return popupConfig;};SGPBPopup.unserialize=function(data)
{var $global=(typeof window!=='undefined'?window:global);var utf8Overhead=function(str){var s=str.length;for(var i=str.length-1;i>=0;i--){var code=str.charCodeAt(i)
...
{var popupId=currentData.data('id');var popupData=currentData.data('options');var events=currentData.data('events');SgpbEventListener.reopenAfterFormSubmission(popupData);var popupObj=new SGPBPopup();popupObj.setPopupId(popupId);popupObj.setPopupData(popupData);for(var i in events){var obj=new this;obj.setPopupObj(popupObj);obj.eventListener(events[i]);}};SgpbEventListener.prototype.eventListener=function(eventData)
...
{var that=listenerObj;var popupObj=that.getPopupObj();var popupOptions=popupObj.getPopupData();var popupId=parseInt(popupObj.id);popupId=listenerObj.filterPopupId(popupId);var targetClick=jQuery('a[href*=#sg-popup-id-'+popupId+'], .sg-popup-id-'+popupId+', .sgpb-popup-id-'+popupId);if(!targetClick.length){return false;}
...
var popupOptions=popupObj.getPopupData();var popupId=parseInt(popupObj.id);popupId=listenerObj.filterPopupId(popupId);var hoverSelector=jQuery('.sg-popup-hover-'+popupId+', .sgpb-popup-id-'+popupId+'[data-popup-event="hover"]');if(!hoverSelector){return false;}
...
{var that=listenerObj;var popupObj=that.getPopupObj();if(!popupObj){return false;}
var popupOptions=popupObj.getPopupData();var popupId=parseInt(popupObj.id);var htmlSelectorPopupId=listenerObj.filterPopupId(popupId);var confirmSelector=jQuery('.sg-confirm-popup-'+htmlSelectorPopupId);if(!confirmSelector){return false;}
