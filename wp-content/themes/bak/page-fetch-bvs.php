...
function fetchProduct($url){
    ...
    $content= $html->find('#TabbedPanels1 .TabbedPanelsContentGroup', 0);
    ...
    $entry = $html->find('.vs_center_content', 0);
    $d_price = $entry->find('.vs_product_spec_value', 2);
    $d_price = $d_price->plaintext;
    $d_price = str_replace('.', '', $d_price);
    $d_price = intval($d_price);
    

    $s_price = $entry->find('.vs_product_detail_price', 0);
    $s_price = $s_price->plaintext;
    $s_price = str_replace('.', '', $s_price);
    $s_price = intval($s_price);
    
    
    $img = $html->find('#large_img_div0 #large_img', 0);
    $image_url = $img->src;
