<?php
/*
  Template Name: Export to Facebook Feed csv
 */
?>
...
$objPHPExcel->getProperties()->setCreator("Bếp An Khang")
							 ...
							 ->setDescription("Danh sách sản phẩm Sẽ up lên Facebook Feed");
   ...
   $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A1', 'id')
                ->setCellValue('B1', 'availability')
                ->setCellValue('C1', 'condition')
                ->setCellValue('D1', 'description')
                ->setCellValue('E1', 'image_link')
    ...
    foreach($products as $product){
        ...
        $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue('B'.$i, 'in stock')
                ->setCellValue('C'.$i, 'new')
                ->setCellValue('D'.$i, mb_substr(strip_tags($product->post_content), 0, 2000))
                ->setCellValue('E'.$i, get_the_post_thumbnail_url($product->ID))
                ->setCellValue('F'.$i, get_permalink($product->ID))
                ->setCellValue('G'.$i, $product->post_title)
                ->setCellValue('H'.$i, $s_price)
                ->setCellValue('I'.$i, $brand->name);
